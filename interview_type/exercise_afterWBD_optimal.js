...
function cleanseUserData(rawUsers) {
    ...
    return rawUsers
    .filter(subarr=>{return (typeof(subarr.id)==='string')&&(subarr.id)})
    .map(subObj=>{
        const fullName_formater= (name)=>{
            return name
            .split(' ')
            .filter(Boolean)
        ...
        const fullName = subObj.fullName ? fullName_formater(subObj.fullName) : "Anonymous User";
        const email = subObj.email.toLowerCase().trim();


        const prefthem = ["dark","light"].includes(subObj?.preferences?.theme)? subObj.preferences.theme : 'light';
        const notifications = ["on"].includes(subObj?.preferences?.notifications)? subObj.preferences.notifications : false;
        const prefer={theme:prefthem,notifications};
        const isOnline = ((!subObj.lastActive) || ((new Date(Date.now()) - new Date(subObj.lastActive)) > 900000))
    ? false // If the condition is TRUE (inactive/missing time), set isOnline to false
    : true; // Otherwise, set isOnline to true
